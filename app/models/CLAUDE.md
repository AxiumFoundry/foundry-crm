...
| `CrmInquiry` | Lead capture | `contact_name`, `company_name`, `email`, `status` |
...
- Sources: `health_check`, `chat`, `referral`, `website`, `manual`, `crm_inquiry`
- Has many: projects, invoices, health_check_submissions, crm_inquiries, chat_conversations
...
- Belongs to: contact (optional, auto-created)
- Callbacks:
  - `after_create :send_confirmation_email` (via CrmInquiryMailer)
  - `after_create :notify_admin` (via CrmInquiryMailer)
  - `after_create :find_or_create_contact` - auto-creates Contact from form data
