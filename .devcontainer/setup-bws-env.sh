...
if [ -n "$BWS_ACCESS_TOKEN" ] && command -v bws >/dev/null 2>&1; then
  ...
  bws run -- sh -c 'env | grep -E "^FOUNDRY_CRM_" | while IFS= read -r line; do
    key="${line%%=*}"
    value="${line#*=}"
    printf "export %s=\"%s\"\n" "$key" "$value"
  done' > "$BWS_ENV_FILE"

  # Add mapping for FOUNDRY_CRM_ prefixed variables
  echo '# Map FOUNDRY_CRM_ prefixed variables' >> "$BWS_ENV_FILE"
  echo '[ -n "$FOUNDRY_CRM_RAILS_MASTER_KEY" ] && export RAILS_MASTER_KEY="$FOUNDRY_CRM_RAILS_MASTER_KEY"' >> "$BWS_ENV_FILE"
  echo '[ -n "$FOUNDRY_CRM_HONEYBADGER_API_KEY" ] && export HONEYBADGER_API_KEY="$FOUNDRY_CRM_HONEYBADGER_API_KEY"' >> "$BWS_ENV_FILE"
  echo '[ -n "$FOUNDRY_CRM_BREVO_SMTP_KEY" ] && export BREVO_SMTP_KEY="$FOUNDRY_CRM_BREVO_SMTP_KEY"' >> "$BWS_ENV_FILE"
  echo '[ -n "$FOUNDRY_CRM_OPENAI_API_KEY" ] && export OPENAI_API_KEY="$FOUNDRY_CRM_OPENAI_API_KEY"' >> "$BWS_ENV_FILE"
